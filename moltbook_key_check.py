...
def _probe(key, timeout=10):
    """Return True if key authenticates successfully."""
    ...
    try:
        ...
        if e.code == 401:
            return False
        _log(f"probe HTTPError {e.code}: {e.reason}")
        return False
    except Exception as e:
        _log(f"probe error: {e!r}")
        return False
